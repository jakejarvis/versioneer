...
export const sources = sqliteTable("sources", {
  ...
  status: text("status", { enum: ["active", "paused", "disabled", "error"] }).notNull().default("active"),
...
export const sourceFetches = sqliteTable("source_fetches", {
  id: text("id").primaryKey(),
  sourceId: text("source_id").notNull().references(() => sources.id),
  fetchStatus: text("fetch_status", { enum: ["success", "not_modified", "error", "timeout"] }).notNull(),
...
export const parserRuns = sqliteTable("parser_runs", {
  id: text("id").primaryKey(),
  sourceFetchId: text("source_fetch_id").notNull().references(() => sourceFetches.id),
  parserKey: text("parser_key").notNull(),
