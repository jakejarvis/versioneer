...
export const releases = sqliteTable("releases", {
  ...
  channel: text("channel", { enum: ["stable", "beta", "nightly"] }).notNull().default("stable"),
  releasedAt: text("released_at"),
  isPrerelease: integer("is_prerelease", { mode: "boolean" }).notNull().default(false),
  sourceConfidence: integer("source_confidence"),
  status: text("status", { enum: ["active", "retracted", "superseded", "draft"] }).notNull().default("active"),
...
export const releaseObservations = sqliteTable("release_observations", {
  id: text("id").primaryKey(),
  parserRunId: text("parser_run_id").notNull().references(() => parserRuns.id),
  appId: text("app_id").notNull().references(() => apps.id),
...
export const artifacts = sqliteTable("artifacts", {
  ...
  artifactType: text("artifact_type", { enum: ["zip", "dmg", "pkg", "appcast_enclosure", "other"] }).notNull(),
  ...
  signatureStatus: text("signature_status", { enum: ["unknown", "valid", "invalid", "missing"] }).default("unknown"),
  notarizationStatus: text("notarization_status", { enum: ["unknown", "notarized", "not_notarized"] }).default("unknown"),
...
export const artifactContents = sqliteTable("artifact_contents", {
  id: text("id").primaryKey(),
  ...
  index("idx_artifact_contents_artifact_id").on(table.artifactId),
]);

export const appLatestReleases = sqliteTable("app_latest_releases", {
  id: text("id").primaryKey(),
  appId: text("app_id").notNull().references(() => apps.id),
  channel: text("channel", { enum: ["stable", "beta", "nightly"] }).notNull().default("stable"),
  releaseId: text("release_id").notNull().references(() => releases.id),
  ...
  versionRaw: text("version_raw").notNull(),
  releasedAt: text("released_at"),
  decisionSource: text("decision_source", { enum: ["pipeline", "override", "manual"] }).notNull().default("pipeline"),
  confidence: integer("confidence"),
  updatedAt: text("updated_at").notNull(),
}, (table) => [
  uniqueIndex("idx_latest_app_channel").on(table.appId, table.channel),
]);

export const installRules = sqliteTable("install_rules", {
  id: text("id").primaryKey(),
  appId: text("app_id").notNull().references(() => apps.id),
  ...
  index("idx_install_rules_app_id").on(table.appId),
]);
