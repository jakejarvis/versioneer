...
export const apps = sqliteTable("apps", {
  ...
  status: text("status", { enum: ["active", "deprecated", "merged", "unlisted"] }).notNull().default("active"),
...
export const appAliases = sqliteTable("app_aliases", {
  id: text("id").primaryKey(),
  appId: text("app_id").notNull().references(() => apps.id),
  aliasType: text("alias_type", {
    enum: ["bundle_id", "name", "team_id", "sparkle_feed", "homepage", "download_pattern", "github_repo"],
  }).notNull(),
...
export const appMatchRules = sqliteTable("app_match_rules", {
  id: text("id").primaryKey(),
  appId: text("app_id").notNull().references(() => apps.id),
  ...
  index("idx_match_rules_app_id").on(table.appId),
]);
