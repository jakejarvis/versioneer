...
import {
  ...
  inventoryIconUploadKindValues,
  inventoryIconUploadStatusValues,
...
export const inventoryIconUploadRequests = sqliteTable(
  "inventory_icon_upload_requests",
  {
    id: text("id").primaryKey(),
    submissionId: text("submission_id").notNull(),
    kind: text("kind", { enum: inventoryIconUploadKindValues }).notNull(),
    status: text("status", { enum: inventoryIconUploadStatusValues }).notNull().default("pending"),
  ...
  (table) => [
    index("idx_inventory_icon_upload_submission").on(table.submissionId),
    index("idx_inventory_icon_upload_status_expires").on(table.status, table.expiresAt),
    index("idx_inventory_icon_upload_app").on(table.appId),
    index("idx_inventory_icon_upload_lookup").on(table.lookupKey),
  ],
);
