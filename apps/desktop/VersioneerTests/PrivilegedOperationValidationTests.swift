...
struct PrivilegedOperationValidationTests {
    ...
    @Test func rejectsStagingOutsideAllowedRoot() throws {
        let sandbox = try TestSandbox()
        try sandbox.createDirectory(at: sandbox.allowedStagingRoot)
        let externalStaging = sandbox.root.appendingPathComponent("elsewhere/exec_test", isDirectory: true)
    ...
    @Test func rejectsSymlinkedDestinationPaths() throws {
        ...
        let symlinkedAppsDirectory = sandbox.root.appendingPathComponent("LinkedApps", isDirectory: true)
        try sandbox.createDirectory(at: realAppsDirectory)
        try FileManager.default.createSymbolicLink(at: symlinkedAppsDirectory, withDestinationURL: realAppsDirectory)
    ...
    @Test func rejectsSymlinkedStagingPaths() throws {
        ...
        let symlinkedStagingRoot = sandbox.root.appendingPathComponent("LinkedInstallStaging", isDirectory: true)
        try FileManager.default.createSymbolicLink(at: symlinkedStagingRoot, withDestinationURL: sandbox.allowedStagingRoot)
        ...
        let request = PrivilegedOperationRequest(
            executionId: "exec_staging_symlink",
            stagingDirectoryPath: symlinkedStagingRoot.appendingPathComponent("exec_staging_symlink", isDirectory: true).path
        )
    ...
    @Test func rejectsReplaceSourcesThatAreNotAppBundles() throws {
        let sandbox = try TestSandbox()
        let context = try sandbox.makeReplaceContext(executionId: "exec_bad_source", sourceName: "Test.pkg")
    ...
    @Test func rejectsPackageSourcesThatAreNotPackages() throws {
        let sandbox = try TestSandbox()
        let context = try sandbox.makePackageContext(executionId: "exec_bad_pkg", packageName: "payload.zip")
...
private struct TestSandbox {
    ...
    func makeReplaceContext(
        ...
        let sourceURL = payloadDirectory.appendingPathComponent(sourceName, isDirectory: sourceName.hasSuffix(".app"))
