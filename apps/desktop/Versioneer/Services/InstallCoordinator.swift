...
final class InstallCoordinator {
    ...
    private func reportStatus(
        ...
        let renderedDetails: String? = if let detailsJSONString {
            detailsJSONString
        } else if let details {
    ...
    private func downloadArtifact(plan: InstallPlan, to stagingDirectory: URL) async throws -> URL {
        ...
        let fileExtension = if !downloadURL.pathExtension.isEmpty {
    ...
    private func verifyAppBundle(
        ...
        if plan.localVerification.requireSignature || plan.localVerification.requireTeamIdMatch {
            ...
            summary.observedTeamId = parseCodesignField("TeamIdentifier", from: combinedOutput) ?? summary.observedTeamId
        }
    ...
    private func verifyPackage(
        ...
        if plan.localVerification.requireTeamIdMatch {
            guard summary.observedTeamId == plan.artifact?.expectedTeamId else {
                throw InstallError.verificationFailed("Installer team identifier did not match the install plan")
            }
            summary.teamIdMatch = true
        }
    ...
    private func verifyArtifactHashIfNeeded(
        ...
        guard let expectedHash else {
            throw InstallError.verificationFailed("Install plan required a SHA-256 hash but none was supplied")
        }
        ...
        guard actualHash == expectedHash.lowercased() else {
            throw InstallError.verificationFailed("Downloaded artifact hash did not match the install plan")
        }
    }
...
private final class ExternalSparkleInstaller: NSObject, SPUUpdaterDelegate {
    ...
    func updater(
        ...
        if let error {
            ...
            resume(with: .failure(InstallError.downloadFailed("Sparkle did not find an applicable update")))
        } else {
            resume(with: .success(()))
        }
    }
