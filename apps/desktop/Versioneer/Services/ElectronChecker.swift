...
actor ElectronChecker {
    ...
    func checkAll(apps: [InstalledApp]) async -> [String: ElectronResult] {
        ...
        return await withTaskGroup(of: (String, ElectronResult?).self) { group in
            ...
            Logger.electron.info("Electron checks complete: \(results.count)/\(electronApps.count) succeeded")
            return results
        }
    }
    ...
    private func checkGitHub(updateUrl: String) async -> ElectronResult? {
        ...
        do {
            ...
            let downloadUrl: String? = if let assets = json["assets"] as? [[String: Any]] {
            ...
            Logger.electron.debug("Failed to fetch GitHub release for \(updateUrl): \(error.localizedDescription)")
            return nil
        }
    }
    ...
    private func checkGeneric(updateUrl: String) async -> ElectronResult? {
        ...
        let feedUrl = updateUrl.hasSuffix("/")
        ...
        do {
            ...
            Logger.electron.debug("Failed to fetch latest-mac.yml from \(feedUrl): \(error.localizedDescription)")
            return nil
        }
    }
