    ...
    func scan() async -> [InstalledApp] {
        ...
        for directory in Self.scanDirectories {
            ...
            do {
                ...
                Logger.appScanner.error("Failed to scan directory \(directory.path): \(error.localizedDescription)")
            }
        }

        let elapsed = CFAbsoluteTimeGetCurrent() - startTime
        Logger.appScanner.info("Scan complete: found \(results.count) apps in \(String(format: "%.2f", elapsed))s")
