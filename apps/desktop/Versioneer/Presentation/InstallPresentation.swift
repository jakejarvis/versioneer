    ...
    private static func banners(
        ...
        if result.install.eligibility == .requiresWarning {
            banners.append(Banner(
                ...
                detail: "Versioneer will still run full local verification before installing this update.",
                tone: .warning
            ))
        }
        ...
        if result.install.requiresAdmin {
            banners.append(Banner(
                ...
                detail: "This update needs Versioneer’s privileged helper to write into a protected location.",
                tone: .neutral
            ))
        }
