...
private struct GeneralSettingsTab: View {
    ...
    private var updateStatusTitle: String {
        if selfUpdateService.isAvailable {
            return selfUpdateService.automaticallyChecksForUpdates ? "Automatic Checks On" : "Manual Checks Only"
        }
        return "Unavailable"
    }
    ...
    private var updateStatusSymbol: String {
        if selfUpdateService.isAvailable {
            return selfUpdateService.automaticallyChecksForUpdates ? "arrow.trianglehead.2.clockwise.circle.fill" : "hand.raised.circle"
        }
