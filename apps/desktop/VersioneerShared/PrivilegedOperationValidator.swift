...
nonisolated enum PrivilegedOperationValidationError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .sourcePathInvalid(let message), .destinationPathInvalid(let message), .backupPathInvalid(let message):
            message
...
nonisolated struct PrivilegedOperationValidator {
    ...
    func validate(request: PrivilegedOperationRequest) throws -> ValidatedPrivilegedOperation {
        guard executionIdRegex.firstMatch(
        ...
        let expectedStagingDirectory = allowedStagingRoot.appendingPathComponent(request.executionId, isDirectory: true)
        ...
        guard stagingDirectory.path == expectedStagingDirectory.standardizedFileURL.path,
              isDescendant(stagingDirectory, of: allowedRoot) else {
        ...
        case .installPackage:
            guard manifest.installTarget == "/", manifest.destinationPath == "/" else {
                throw PrivilegedOperationValidationError.unsupportedInstallTarget(manifest.installTarget ?? manifest.destinationPath)
            }
        ...
        case .replaceApp:
            guard let destinationURL,
                  let backupRelativePath = manifest.backupRelativePath else {
        ...
        case .installPackage:
            guard manifest.installTarget == "/" else {
                throw PrivilegedOperationValidationError.unsupportedInstallTarget(manifest.installTarget ?? "")
            }
            backupURL = nil
        }
    ...
    private func validateRelativePath(
        ...
        guard !relativePath.isEmpty,
            ...
            throw errorBuilder("The privileged install manifest contained an invalid relative source path.")
        }
        ...
        guard canonicalURL.pathExtension == expectedExtension else {
            throw errorBuilder("The privileged install source must be a .\(expectedExtension) path inside staging.")
        }
    ...
    private func canonicalizeExistingURL(
        ...
        do {
            return try canonicalizeURL(url, expectedDirectory: expectedDirectory, allowsMissingLeaf: false)
        ...
        if fileManager.fileExists(atPath: standardizedURL.path, isDirectory: &isDirectory) {
            guard isDirectory.boolValue == expectedDirectory else {
                throw (expectedDirectory
                    ? PrivilegedOperationValidationError.sourcePathInvalid("Expected a bundle directory at \(standardizedURL.path).")
                    : PrivilegedOperationValidationError.sourcePathInvalid("Expected a file at \(standardizedURL.path)."))
            }
        } else if !allowsMissingLeaf {
            throw PrivilegedOperationValidationError.sourcePathInvalid("Expected an existing path at \(standardizedURL.path).")
        } else {
